...
export function Admin({ username }: { username: string }) {
  ...
  async function confirmBooking(row: RecordData) {
    ...
    if (!window.confirm('¿Registrar que pagó la reserva para el turno de ' + recordLabel(data, 'booking-drafts', row) + ' para el ' + display(data, 'date', row.date) + ' a las ' + row.bookingTime + '? Confirmá que recibiste el pago fuera de la plataforma. El horario quedará reservado.')) return;
    setBusy(true); setError('');
    try { await requestApi('booking-drafts', 'POST', {}, row.id + '/confirm'); await load(); setNotice('Pago de reserva registrado. El horario quedó reservado.'); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudo confirmar.'); }
    finally { setBusy(false); }
  }
  async function recordTotalPayment(row: RecordData) {
    if (busy || loading || row.status !== 'CONFIRMED' || row.paymentStatus === 'TOTAL_PAID') return;
    if (!window.confirm('¿Registrar que ' + recordLabel(data, 'booking-drafts', row) + ' pagó el total? Confirmá que recibiste el pago completo fuera de la plataforma.')) return;
    setBusy(true); setError('');
    try { await requestApi('booking-drafts', 'POST', {}, row.id + '/total-payment'); await load(); setNotice('Pago total registrado.'); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'No se pudo registrar el pago total.'); }
    finally { setBusy(false); }
  }
  ...
  const requestRows: RecordData[] = data['booking-drafts'].map(row => {
    ...
    return { ...row, status: row.status ?? 'PENDING_CONFIRMATION', paymentStatus: row.paymentStatus ?? (row.status === 'CONFIRMED' ? 'RESERVATION_PAID' : 'PENDING'), bookingTime: start && end ? time(start) + ' – ' + time(end) : 'Sin horario elegido' };
  });
  ...
  const rows = section === 'overview' ? [] : (section === 'booking-drafts' ? requestRows : data[section]).filter(row => {
    ...
    if (section === 'booking-drafts') {
      ...
      return matchesSearch && (!state || row.paymentStatus === state) && (!venueId || row.venueId === venueId) && (!sportId || row.sportId === sportId)
