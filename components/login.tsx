...
export function Login() {
  ...
  return <main className="login"><section className="login-story"><div className="brand"><Image className="brand-icon" src="/turnerop-backoffice-icon.svg" width={35} height={35} alt="" /> turnero<span className="brand-tag">ADMIN</span></div><div><span className="eyebrow">TU OPERACIÓN, EN UN SOLO LUGAR</span><h1>Más juego.<br />Mejor gestión.</h1><p>Administrá tus sedes, organizá la disponibilidad y mantené cada detalle al día.</p><div className="court" aria-hidden="true"><div /><span /></div></div><small>Panel administrativo de polideportivos</small></section><section className="login-form"><form onSubmit={submit}><span className="eyebrow">BIENVENIDO AL BACKOFFICE</span><h2>Ingresá a tu espacio</h2><p>Usá tus credenciales de administrador para continuar.</p><label>Usuario<input name="username" autoComplete="username" required maxLength={80} autoFocus /></label><label>Contraseña<input name="password" type="password" autoComplete="current-password" required maxLength={512} /></label>{error && <div role="alert" className="notice error">{error}</div>}<button className="primary" disabled={busy}>{busy ? 'Ingresando…' : 'Ingresar al panel →'}</button><small>Acceso exclusivo para administradores autorizados.</small></form></section></main>;
}
