...
export const definitions: Record<Resource, { title: string; singular: string; description: string; icon: string; fields: Field[]; columns: string[] }> = {
  ...
  'booking-drafts': { title: 'Solicitudes', singular: 'solicitud', description: 'Borradores de alquiler. La confirmación se gestiona con cada sede.', icon: '▧', columns: ['renterFirstName', 'renterLastName', 'renterPhone', 'venueId', 'date'], fields: [{ key: 'sportId', label: 'Deporte', relation: 'sports' }, { key: 'renterFirstName', label: 'Nombre', optional: true, max: 80 }, { key: 'renterLastName', label: 'Apellido', optional: true, max: 80 }, { key: 'renterPhone', label: 'Teléfono', type: 'tel', optional: true }, { key: 'zoneId', label: 'Zona', relation: 'zones', optional: true }, { key: 'venueId', label: 'Sede', relation: 'venues', optional: true }, { key: 'date', label: 'Fecha', type: 'date', optional: true }, { key: 'slotId', label: 'Turno', relation: 'slots', optional: true }] },
};
...
export const labels: Record<string, string> = { name: 'Nombre', icon: 'Ícono', isActive: 'Estado', zoneId: 'Zona', venueId: 'Sede', sportId: 'Deporte', address: 'Dirección', startsAt: 'Inicio', endsAt: 'Fin', status: 'Estado', renterFirstName: 'Nombre', renterLastName: 'Apellido', renterPhone: 'Teléfono', date: 'Fecha' };
...
export function display(data: Dataset, key: string, value: RecordData[string]): string {
  ...
  if (key === 'status') return value === 'AVAILABLE' ? 'Disponible' : 'Bloqueado';
