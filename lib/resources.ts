...
export const definitions: Record<Resource, { title: string; singular: string; description: string; icon: string; fields: Field[]; columns: string[] }> = {
  ...
  'booking-drafts': { title: 'Solicitudes', singular: 'solicitud', description: 'Registrá el pago de la reserva y luego el pago total. Los pagos se realizan fuera de la plataforma.', icon: '▧', columns: ['renterFirstName', 'renterLastName', 'renterPhone', 'venueId', 'sportId', 'date', 'bookingTime', 'paymentStatus'], fields: [{ key: 'sportId', label: 'Deporte', relation: 'sports' }, { key: 'renterFirstName', label: 'Nombre', optional: true, max: 80 }, { key: 'renterLastName', label: 'Apellido', optional: true, max: 80 }, { key: 'renterPhone', label: 'Teléfono', type: 'tel', optional: true }, { key: 'zoneId', label: 'Zona', relation: 'zones', optional: true }, { key: 'venueId', label: 'Sede', relation: 'venues', optional: true }, { key: 'date', label: 'Fecha', type: 'date', optional: true }, { key: 'slotId', label: 'Turno', relation: 'slots', optional: true }] },
};
...
export const labels: Record<string, string> = { name: 'Nombre', icon: 'Ícono', isActive: 'Estado', zoneId: 'Zona', venueId: 'Sede', sportId: 'Deporte', address: 'Dirección', startsAt: 'Inicio', endsAt: 'Fin', status: 'Estado', paymentStatus: 'Estado de pago', renterFirstName: 'Nombre', renterLastName: 'Apellido', renterPhone: 'Teléfono', date: 'Fecha', bookingTime: 'Horario' };
...
export function display(data: Dataset, key: string, value: RecordData[string]): string {
  ...
  if (key === 'paymentStatus') return ({ PENDING: 'Pendiente', RESERVATION_PAID: 'Pagó reserva', TOTAL_PAID: 'Pagó total' } as Record<string, string>)[String(value)] ?? String(value);
  if (key === 'status') return ({ AVAILABLE: 'Disponible', UNAVAILABLE: 'Bloqueado', RESERVED: 'Reservado', CONFIRMED: 'Pagó reserva', PENDING_CONFIRMATION: 'Pendiente' } as Record<string, string>)[String(value)] ?? String(value);
